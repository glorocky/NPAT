...
from services.bootstrap import create_market_service
...
if "market_service" not in st.session_state:
    st.session_state.market_service = create_market_service()

if "paper_trading_service" not in st.session_state:
    st.session_state.paper_trading_service = PaperTradingService()

service = st.session_state.market_service
paper_service = st.session_state.paper_trading_service


@st.fragment(
    run_every=(
        settings["refresh_interval"] if settings["auto_refresh"] else None
    )
)
def render_live_dashboard() -> None:
    ...
    render(snapshot, service, paper_service, settings)
